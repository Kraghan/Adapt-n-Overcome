Shader "Space"
{
	Properties
	{
		...
		_Direction("Direction", Vector) = (2,1,0,0)
	}
...
#include "UnityCG.cginc"
	...
	fixed4 frag(VertexOutput i) : SV_Target
	{
		...
		from += fixed3(-time*_Direction.x,-time*_Direction.y,-2.);
		//from.xz*=rot1;
		//from.xy*=rot2;

		//volumetric rendering
		fixed3 pw = fixed3(sin(time / 6.)*0.08 + 1., sin(time / 6.)*0.08 + 1., sin(time / 6.)*0.08 + 1.);
		fixed s = 0.1,fade = 1.;
		...
		for (int r = 0; r<_Steps; r++)
		{
			...
			{
				p = pow(abs(p), pw) / dot(p, p) - _Formuparam;
